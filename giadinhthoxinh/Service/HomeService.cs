...
namespace giadinhthoxinh.Service
{
    public class HomeService : IHomeService
    {
        ...
        public string CreateMedicalBill(PatientInfor medicalBillInfor)
        {
            ...
            AddRow(table, 0, 0, 0, "SỞ Y TẾ THÀNH PHỐ HÀ NỘI", 9.5f, null, null, null, HorizontalAlignment.Center);
            AddRow(table, 0, 1, 0, "PHIẾU KHÁM BỆNH", 12.5f, true, null, null, HorizontalAlignment.Center);
            AddRow(table, 0, 2, 0, "Số phiếu: ", 9.5f, null, null, null, HorizontalAlignment.Center);
            AddRow(table, 1, 0, 0, "BỆNH VIỆN ĐẠI HỌC PHENIKAA", 9f, true, null, null, HorizontalAlignment.Center);
            AddRow(table, 1, 2, 0, "Mã NB: ", 9.5f, null, null, null, HorizontalAlignment.Center);
            AddRow(table, 2, 0, 0, "Hotline: ..........", 9.5f, null, null, Color.Red, HorizontalAlignment.Center);
            ...
            #region administrative 
            AddRow(table, 3, 0, 0, "I. HÀNH CHÍNH: ", 11, true);
            ...
            row13.Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            row13.Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            AddTextToCell(table, 12, 0, 10, HorizontalAlignment.Left, new string[] { "13. Chẩn đoán của nơi giới thiệu ", "(nếu có): ", $"{medicalBillInfor.DiagnosisOfReferralSite}" }, new float[] { 11, 11, 11 }, new bool[] { false, false, false }, new bool[] { false, true, false }); ;
            #endregion
            ...
            #region to solve
            int rowNumber = 35;
            AddRow(table, rowNumber, 0, 0, "III. XỬ TRÍ", 12, true);
            AddRow(table, rowNumber++, 2, 0, "Hà Nội, ngày\ttháng\tnăm 20", 10, null, true, null, HorizontalAlignment.Center);
            AddRow(table, rowNumber++, 2, 0, "BÁC SỸ KHÁM BỆNH", 11, true, null, null, HorizontalAlignment.Center);
            AddRow(table, rowNumber++, 2, 0, "(Ký và ghi rõ họ tên)", 11, null, true, null, HorizontalAlignment.Center);
            AddRow(table, rowNumber++, 0, 0, "*Ghi chú:", 12, true);
            table.Rows[rowNumber].Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            table.Rows[rowNumber].Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            AddRow(table, rowNumber, 0, 0, "- Uống thuốc theo đơn. Có gì bất thường đến viện kiểm tra lại.\r\n- Người bệnh nhận đơn thuốc tại phòng khám bác sỹ, lĩnh thuốc tại quầy thuốc BHYT hoặc mua thuốc tại các nhà thuốc bệnh viện.\r\n", 11, null, true);
        ...
        public void AddTextToCell(Table table, int rowIndex, int cellIndex, int? LeftIndent = null, HorizontalAlignment? alignment = null, string[]? texts = null, float[]? fontSizes = null, bool[]? bold = null, bool[]? italic = null)
        {
        ...
        public async Task<string> CreateMedicalBill1(MedicalBillInfor medicalBillInfor)
        {
            ...
            AddRow(table, 0, 0, 0, "SỞ Y TẾ THÀNH PHỐ HÀ NỘI", 9.5f, null, null, null, HorizontalAlignment.Center);
            AddRow(table, 0, 1, 0, "PHIẾU KHÁM BỆNH", 12.5f, true, null, null, HorizontalAlignment.Center);
            AddRow(table, 0, 2, 0, "Số phiếu: ", 9.5f, null, null, null, HorizontalAlignment.Center, patientInfor?.OrderNumber?.ToString());
            AddRow(table, 1, 0, 0, "BỆNH VIỆN ĐẠI HỌC PHENIKAA", 9f, true, null, null, HorizontalAlignment.Center, "\r\nPHENIKAA HOÀNG NGÂN", UnderlineStyle.Single, true);
            AddRow(table, 1, 2, 0, "Mã NB: ", 9.5f, null, null, null, HorizontalAlignment.Center, medicalBillInfor.PatientId.ToString().Substring(0, 8).ToString());
            AddRow(table, 2, 0, 0, "Hotline: ..........", 9.5f, null, null, Color.Red, HorizontalAlignment.Center);

            Paragraph type = table.Rows[2].Cells[1].AddParagraph();
            TextRange normalTypeTR = type.AppendText("Thường  ");
            normalTypeTR.CharacterFormat.FontSize = 9.5f;
            TextRange CheckboxNormalTR = type.AppendText(patientInfor?.Type == 1 ? "\u2611" : "\u2610"); // Add checkbox empty (Unicode)
            CheckboxNormalTR.CharacterFormat.FontSize = 20;
            TextRange emergencyType = type.AppendText("     Cấp cứu   ");
            emergencyType.CharacterFormat.FontSize = 9.5f;
            TextRange CheckboxEmergencyTR = type.AppendText(patientInfor.Type == 2 ? "\u2611" : "\u2610");
            CheckboxEmergencyTR.CharacterFormat.FontSize = 20;
            type.Format.HorizontalAlignment = HorizontalAlignment.Center;
            AddRow(table, 2, 2, 0, "Barcode ", 9.5f, true, null, Color.Red, HorizontalAlignment.Center);
            #endregion
            ...
            #region administrative 
            AddRow(table, 3, 0, 0, "I. HÀNH CHÍNH: ", 11, true);
            AddRow(table, 4, 0, 10, "1. Họ và tên (in hoa): ", 11, null, null, null, null, patientInfor.Username?.ToUpper());
            AddRow(table, 4, 1, 20, $"2. Ngày sinh: {patientInfor.Birthday?.Day.ToString()}/{patientInfor.Birthday?.Month.ToString()}/{patientInfor.Birthday?.Year.ToString()}", 11, null, null, null, null);
            AddRow(table, 4, 2, 10, "Tuổi: ", 11, null, null, null, null, patientInfor.Age?.ToString());
            Paragraph genderAndJob = table.Rows[5].Cells[0].AddParagraph();
            genderAndJob.Format.LeftIndent = 10;
            TextRange genderTR = genderAndJob.AppendText($"3. Giới tính: {patientInfor.Gender?.ToString()}   ");
            genderTR.CharacterFormat.FontSize = 11;
            TextRange jobTR = genderAndJob.AppendText($"4. Nghề nghiệp: {patientInfor.Job?.ToString()}");
            jobTR.CharacterFormat.FontSize = 11;
            AddRow(table, 5, 1, 20, "5. Dân tộc:", 11, null, null, null, null, patientInfor.Ethnic?.ToString());
            AddRow(table, 5, 2, 10, "6. Quốc tịch:", 11, null, null, null, null, patientInfor.Nationality?.ToString());
            AddRow(table, 6, 0, 10, "7. Địa chỉ: ", 11, null, null, null, null, patientInfor.Address?.ToString());
            AddRow(table, 7, 0, 10, "8. Nơi làm việc: ", 11, null, null, null, null, patientInfor.Workplace?.ToString());
            AddRow(table, 7, 1, 20, "SĐT: ", 11, null, null, null, null, patientInfor.Phone?.ToString());
            #region subject
            table.Rows[8].Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            table.Rows[8].Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            Paragraph objectPatient = table.Rows[8].Cells[0].AddParagraph();
            ...
            healthInsuranceTR.CharacterFormat.FontSize = 11f;
            healthInsuranceTR.CharacterFormat.Italic = true;
            TextRange checkboxHealthInsurance = objectPatient.AppendText(patientInfor.Subject == 1 ? "\u2611  " : "\u2610  ");
            checkboxHealthInsurance.CharacterFormat.FontSize = 20;

            TextRange feeTR = objectPatient.AppendText("2.Thu phí ");
            feeTR.CharacterFormat.FontSize = 11f;
            feeTR.CharacterFormat.Italic = true;
            TextRange CheckboxFee = objectPatient.AppendText(patientInfor.Subject == 2 ? "\u2611  " : "\u2610  ");
            CheckboxFee.CharacterFormat.FontSize = 20;

            TextRange freeTR = objectPatient.AppendText("3.Miễn ");
            freeTR.CharacterFormat.FontSize = 11f;
            freeTR.CharacterFormat.Italic = true;
            TextRange checkboxFree = objectPatient.AppendText(patientInfor.Subject == 3 ? "\u2611  " : "\u2610  ");
            checkboxFree.CharacterFormat.FontSize = 20;

            TextRange otherTR = objectPatient.AppendText("4.Khác ");
            otherTR.CharacterFormat.FontSize = 11f;
            otherTR.CharacterFormat.Italic = true;
            TextRange checkboxOther = objectPatient.AppendText(patientInfor.Subject == 4 ? "\u2611  " : "\u2610  ");
            checkboxOther.CharacterFormat.FontSize = 20;
            #endregion
            // Row7
            table.Rows[9].Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            ...
            table.Rows[10].Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            table.Rows[10].Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            AddRow(table, 10, 0, 10, "11. Họ tên, địa chỉ người nhà khi cần báo tin: ", 11, null, null, null, null, patientInfor.FamilyInformation?.ToString());
            AddRow(table, 10, 2, 0, "Số điện thoại: ", 11, null, null, null, null, patientInfor.FamilyInformationPhone?.ToString());

            // Row9
            AddRow(table, 11, 0, 10, "12. (a) Đến khám bệnh lúc: ", 11, null, null, null, null, patientInfor.TimeComeExamination?.ToString("HH:mm"));
            table.Rows[11].Cells[1].CellFormat.HorizontalMerge = CellMerge.Start;
            table.Rows[11].Cells[2].CellFormat.HorizontalMerge = CellMerge.Continue;
            AddRow(table, 11, 1, 0, "12. (b) Bắt đầu khám lúc: ", 11, null, null, null, null, patientInfor.TimeStartExamination?.ToString("HH:mm"));
            // Row10
            TableRow row13 = table.Rows[12];
            row13.Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            row13.Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            AddTextToCell(table, 12, 0, 10, HorizontalAlignment.Left, new string[] { "13. Chẩn đoán của nơi giới thiệu ", "(nếu có): ", $"{patientInfor.DiagnosisOfReferralSite}" }, new float[] { 11, 11, 11 }, new bool[] { false, false, false }, new bool[] { false, true, false }); ;
            #endregion
            ...
            AddRow(table, 15, 0, 10, "1.2 Bệnh sử: ", 11, true);
            AddRow(table, 15, 2, 0, "Mạch:\t\t\tL/phút", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 16, 0, 10, "- ", 11, null, true, null, null, medicalBillInfor?.ReportParamData?.ReportParameterUserPersonalMedicalHistory);
            AddRow(table, 16, 2, 0, "Nhiệt độ:\t\t°C", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 17, 2, 0, "Huyết áp:\t\tmmHg", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 18, 2, 0, "Nhịp thở:\t\tL/phút", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 19, 0, 10, $"1.3 Tiền sử bệnh: ", 11, true);
            AddRow(table, 19, 2, 0, "Cân nặng:\t\tkg", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 20, 0, 10, "- Bản thân: ", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterUserPersonalMedicalHistory);
            AddRow(table, 20, 2, 0, "Chiều cao:\t\tcm", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 21, 0, 10, "- Gia đình: ", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterUserFamilyMedicalHistory);
            AddRow(table, 21, 2, 0, "BMI:", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 22, 0, 10, "1.4 Khám lâm sàng:", 11, true);
            AddRow(table, 22, 2, 0, "SPO2:\t\t\t%", 11, null, true, null, HorizontalAlignment.Left);
            AddRow(table, 23, 0, 10, "- Toàn thân", 11);
            AddRow(table, 24, 0, 10, "- Các bộ phận", 11);
            AddRow(table, 25, 0, 10, "1.5 Chuẩn đoán sơ bộ:", 11, true);
            //AddRow(table, index++, 0, 10, "-", 11);
            AddRow(table, 26, 0, 10, "-", 11);
            AddRow(table, 27, 0, 10, "1.6 Chỉ định cận lâm sàng:", 11, true);
            ...
            AddRow(table, 28, 0, 10, "- Xét nghiệm:", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterIndicationsParaclinicalTest);
            ...
            AddRow(table, 29, 0, 10, "- Chẩn đoán hình ảnh, TDCN:", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterIndicationsParaclinicalCdha);
            AddRow(table, 30, 0, 10, "1.7 Tóm tắt kết quả cận lâm sàng:", 11, true);
            ...
            AddRow(table, 31, 0, 10, "- ", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterSubTest);
            ...
            AddRow(table, 32, 0, 10, "- ", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterSubTest);
            AddRow(table, 33, 0, 10, "1.8 Chẩn đoán xác định:", 11, true);
            ...
            {
                if (i == 0) AddRow(table, 34, 0, 10, "- Bệnh chính: ", 11, null, null, null, null, null);
                ...
                AddRow(table, 34 + i + 1, 2, 0, "", 9.5f, null, null, null, null, $"Mã ICD1:{diagnoses[i].ICDCode}");
            ...
            for (int i = 0; i < diagnosesCount; i++)
            {
                ...
                AddRow(table, 35 + indexIsAddedIntermediately, 2, 0, "", 9.5f, null, null, null, null, $"Mã ICD1: {diagnoses[i].ICDCode}");
            ...
            #region to solve
            //int index = 36;
            //AddRow(table, index, 0, 0, "III. XỬ TRÍ", 12, true);
            //AddRow(table, index++, 2, 0, $"Hà Nội, ngày {medicalBillInfor?.ReportParamData?.ReportParameterUserStartExaminationDateTime?.Day.ToString()} tháng {medicalBillInfor?.ReportParamData?.ReportParameterUserStartExaminationDateTime?.Month.ToString()} năm {medicalBillInfor?.ReportParamData?.ReportParameterUserStartExaminationDateTime?.Year.ToString()}", 10, null, true, null, HorizontalAlignment.Center);
            //AddRow(table, index++, 2, 0, "BÁC SỸ KHÁM BỆNH", 11, true, null, null, HorizontalAlignment.Center);
            //AddRow(table, index++, 2, 0, "(Ký và ghi rõ họ tên)", 11, null, true, null, HorizontalAlignment.Center);
            //AddRow(table, index++, 2, 0, "", 11, null, true, null, HorizontalAlignment.Center, medicalBillInfor?.ReportParamData?.Doctor?.Name);
            //AddRow(table, index++, 0, 0, "*Ghi chú:", 12, true);
            //table.Rows[index++].Cells[0].CellFormat.HorizontalMerge = CellMerge.Start;
            //table.Rows[index].Cells[1].CellFormat.HorizontalMerge = CellMerge.Continue;
            //AddRow(table, index, 0, 0, "- Uống thuốc theo đơn. Có gì bất thường đến viện kiểm tra lại.\r\n- Người bệnh nhận đơn thuốc tại phòng khám bác sỹ, lĩnh thuốc tại quầy thuốc BHYT hoặc mua thuốc tại các nhà thuốc bệnh viện.\r\n", 11, null, true);

            AddRow(table, 36 + indexIsAddedIntermediately, 0, 0, "III. XỬ TRÍ", 12, true);
            ...
            AddRow(table, 38 + indexIsAddedIntermediately, 2, 0, "BÁC SỸ KHÁM BỆNH", 11, true, null, null, HorizontalAlignment.Center);
            AddRow(table, 39 + indexIsAddedIntermediately, 2, 0, "(Ký và ghi rõ họ tên)", 9.5f, null, true, null, HorizontalAlignment.Center);
            ...
            AddRow(table, 43 + indexIsAddedIntermediately, 0, 0, "*Ghi chú:", 12, true);
            ...
            AddRow(table, 44 + indexIsAddedIntermediately, 0, 0, "- Uống thuốc theo đơn. Có gì bất thường đến viện kiểm tra lại.\r\n- Người bệnh nhận đơn thuốc tại phòng khám bác sỹ, lĩnh thuốc tại quầy thuốc BHYT hoặc mua thuốc tại các nhà thuốc bệnh viện.\r\n", 11, null, true);
            #endregion
            // Save to stream
            //MemoryStream stream = new MemoryStream();
            //doc.SaveToStream(stream, FileFormat.Docx2013);
        ...
        //    AddRow(table, 13, 0, 0, "II. THÔNG TIN KHÁM BỆNH", 12, true);
        //    AddRow(table, 14, 0, 0, "1. Y HỌC HIỆN ĐẠI\r\n  1.1. Lý do đến khám:\r\n", 11, true);
        //    AddRow(table, 15, 0, 10, "1.2 Bệnh sử: \r\n- ", 11, true, null, null,null, medicalBillInfor?.ReportParamData?.ReportParameterUserPersonalMedicalHistory);
        //    AddRow(table, 15, 2, 0, "Mạch:\t\t\tL/phút", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 16, 2, 0, "Nhiệt độ:\t\t°C", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 17, 2, 0, "Huyết áp:\t\tmmHg", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 18, 2, 0, "Nhịp thở:\t\tL/phút", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 19, 0, 10, $"1.3 Tiền sử bệnh: \r\n- Bản thân: {medicalBillInfor?.ReportParamData?.ReportParameterUserPersonalMedicalHistory} \r\n-Gia đình: {medicalBillInfor?.ReportParamData?.ReportParameterUserFamilyMedicalHistory}", 11, true);
        //    AddRow(table, 19, 2, 0, "Cân nặng:\t\tkg", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 20, 0, 10, "-", 11);
        //    AddRow(table, 20, 2, 0, "Chiều cao:\t\tcm", 11, null, true, null, HorizontalAlignment.Left);
        //    AddRow(table, 21, 2, 0, "BMI:", 11, null, true, null, HorizontalAlignment.Left);
        ...
        //    AddRow(table, 27, 0, 10, "-", 11);
        //    AddRow(table, 28, 0, 10, "1.6 Chỉ định cận lâm sàng:", 11, true);
        //    AddRow(table, 29, 0, 10, "- Xét nghiệm:", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterIndicationsParaclinicalTest);
        //    AddRow(table, 30, 0, 10, "- Chẩn đoán hình ảnh, TDCN:", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterIndicationsParaclinicalCdha);
        //    AddRow(table, 31, 0, 10, "1.7 Tóm tắt kết quả cận lâm sàng:", 11, true);
        //    AddRow(table, 32, 0, 10, "-", 11, null, null, null, null, medicalBillInfor?.ReportParamData?.ReportParameterSubTest);
        //    AddRow(table, 33, 0, 10, "1.8 Chẩn đoán xác định:", 11, true);
        //    AddRow(table, 34, 0, 10, "- Bệnh chính: \r\n-Bệnh kèm theo:", 11);
